...
test('workspace root is a private command router rather than an Expo app package', () => {
  ...
  assert.equal(
    ...
    'pnpm -F playground test && pnpm -F @tenkit/types test && pnpm -F @tenkit/template-generator test && pnpm -F @tenkit/release-tools test && pnpm -F @tenkit/cli test && pnpm -F @tenkit/web test',
  );
  assert.equal(
