...
assets/<app-variant-slug>/icons/splash-icon-light.png
assets/<app-variant-slug>/icons/splash-icon-dark.png
assets/<app-variant-slug>/icons/favicon.png
assets/<app-variant-slug>/app.icon/icon.json
```

Those paths are validated when `app.config.ts` resolves the selected App Variant.

If an asset changes by App Variant, put it in that App Variant's folder and reference it through App Variant-aware app code. If an asset is identical for all App Variants, put it here and import it from shared code.
