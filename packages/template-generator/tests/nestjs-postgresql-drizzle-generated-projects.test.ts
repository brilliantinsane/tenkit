...
const NESTJS_POSTGRESQL_DRIZZLE_OPTIONS = {
  backend: 'nestjs',
  auth: 'none',
  database: 'postgresql',
  orm: 'drizzle',
...
test('generates the exact Auth-free NestJS PostgreSQL Drizzle shape for every Setup Type', () => {
  for (const setupType of SUPPORTED_GENERATED_SETUP_TYPE_IDS) {
    const tree = generateProject({
      setupType,
      stylingChoice: 'bare',
      packageManager: 'pnpm',
      generatedAppOptions: NESTJS_POSTGRESQL_DRIZZLE_OPTIONS,
    });
    ...
    assert.match(
      ...
      /createPostgresqlDrizzleDatabase.*createBusinessDataRepository\(database\)/s,
    );
    assert.match(
    ...
    assert.match(
      ...
      /inspectPostgresqlTableNames.*app_variant_runtime_tenant_access/s,
    );
    assert.notMatch(
      readVirtualText(tree, 'apps/server/tests/integration.test.ts'),
      /_prisma_migrations/,
    ...
    assert.notMatch(
      generatedText,
      /better-auth|CLERK_|mysql2|packages\/auth|@clerk|sign-in|sign-up|prisma|fastify|prototype/i,
    );
  }
});
