...
test('generates the NestJS Clerk PostgreSQL Drizzle contract for every Setup Type', () => {
  for (const setupType of SUPPORTED_GENERATED_SETUP_TYPE_IDS) {
    ...
    const serverTypeScript = readVirtualText(tree, 'apps/server/tsconfig.json');
    ...
    assert.match(serverTypeScript, /"skipLibCheck": true/);
    ...
    assert.match(businessProfileModule, /createPostgresqlDrizzleDatabase/);
    assert.match(businessProfileModule, /onApplicationShutdown/);
    assert.match(businessProfileModule, /database\.\$client\.end/);
    ...
    assert.notMatch(
      generatedText,
      /better-auth|packages\/auth|prisma|mysql2|express Backend|convex|fastify|prototype/i,
    );
  }
});
