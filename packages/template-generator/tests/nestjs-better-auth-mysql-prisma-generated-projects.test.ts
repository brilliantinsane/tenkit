...
const NESTJS_BETTER_AUTH_MYSQL_PRISMA_OPTIONS = {
  backend: 'nestjs',
  auth: 'better-auth',
  database: 'mysql',
  orm: 'prisma',
...
test('generates the NestJS Better Auth MySQL Prisma contract for every Setup Type', () => {
  for (const setupType of SUPPORTED_GENERATED_SETUP_TYPE_IDS) {
    const tree = generateProject({
      setupType,
      stylingChoice: 'bare',
      packageManager: 'pnpm',
      generatedAppOptions: NESTJS_BETTER_AUTH_MYSQL_PRISMA_OPTIONS,
    });
    ...
    assert.match(readVirtualText(tree, 'apps/server/src/main.ts'), /bodyParser: false/);
    assert.match(
    ...
    assert.match(integrationTest, /inspectMysqlTableNames/);
    assert.match(readVirtualText(tree, 'README.md'), /immediate durable session/);
    assert.match(generatedText, /Auth: better-auth/);
    assert.notMatch(
      generatedText,
      /CLERK_|@clerk|@prisma\/adapter-pg|drizzle|express ownership|convex|fastify|prototype/i,
    );
  }
});
