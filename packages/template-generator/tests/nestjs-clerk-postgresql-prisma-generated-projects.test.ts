...
const NESTJS_CLERK_POSTGRESQL_PRISMA_OPTIONS = {
  backend: 'nestjs',
  auth: 'clerk',
  database: 'postgresql',
  orm: 'prisma',
...
test('generates the NestJS Clerk PostgreSQL Prisma contract for every Setup Type', () => {
  for (const setupType of SUPPORTED_GENERATED_SETUP_TYPE_IDS) {
    const tree = generateProject({
      setupType,
      stylingChoice: 'bare',
      packageManager: 'pnpm',
      generatedAppOptions: NESTJS_CLERK_POSTGRESQL_PRISMA_OPTIONS,
    });
    ...
    assert.match(integrationTest, /inspectPostgresqlTableNames/);
    ...
    assert.notMatch(
      generatedText,
      /better-auth|packages\/auth|drizzle|mysql2|express Backend|convex|fastify|prototype/i,
    );
  }
});
...
test('protects every Setup Type route set across every Styling Choice', () => {
  assertClerkProtectedRoutes(NESTJS_CLERK_POSTGRESQL_PRISMA_OPTIONS);
});
