...
import type { CreateFlowEnvironment } from '../src/create/types';
import {
  buildConfiguratorCommand,
  createDefaultConfiguratorState,
} from '../../../apps/web/lib/configurator';
...
function getPublicCliArguments(command: string): string[] {
  ...
  return flagParts.flatMap((flagPart) => {
    ...
    return [flagPart.slice(0, separator), unwrapShellValue(flagPart.slice(separator + 1))];
  });
}
