...
test('interactive creation resolves NestJS with Better Auth, MySQL, and Prisma', async () => {
  ...
  const result = await runCreateFlow(
    {
      name: 'interactive-nestjs-better-auth-mysql-prisma',
...
test('explicit flags write the protected NestJS MySQL Prisma stack', async () => {
  ...
  const result = await runCreateFlow(
    {
      name: 'nestjs-better-auth-mysql-prisma-app',
      backend: 'nestjs',
      auth: 'better-auth',
      database: 'mysql',
      orm: 'prisma',
  ...
  expect(
    await fs.readJson(
      join(tempRoot, 'nestjs-better-auth-mysql-prisma-app/apps/server/package.json'),
    ),
  ).toMatchObject({
    dependencies: {
  ...
  expect(
    await fs.readJson(
      join(tempRoot, 'nestjs-better-auth-mysql-prisma-app/packages/auth/package.json'),
    ),
  ).toMatchObject({ dependencies: { '@tenkit/db': 'workspace:*', 'better-auth': '1.6.25' } });
  expect(
    await fs.readJson(
      join(tempRoot, 'nestjs-better-auth-mysql-prisma-app/packages/db/package.json'),
    ),
